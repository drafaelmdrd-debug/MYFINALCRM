...
import { Search, MapPin, Phone, User, ExternalLink, Filter, Mail, Trash2 } from 'lucide-react';
...
export function SearchView({
  ...
  const filtered = leads.filter((l) => {
    ...
    return (
      ...
      (l.mailingAddress && l.mailingAddress.toLowerCase().includes(q)) ||
                ...
                filtered.map((lead) => (
                  <tr key={lead.id} className="hover:bg-[#F8F6F1]/60 transition-colors">
                    ...
                    <td className="py-3 px-4 text-[#5E6660]">
                      ...
                      {lead.mailingAddress && (
                        <div className="flex items-center gap-1 text-[11px] text-[#8C948E] mt-0.5 ml-5">
                          <Mail className="w-3 h-3 text-[#B85338] shrink-0" />
                          <span title={lead.mailingAddress}>Mail: {lead.mailingAddress}</span>
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-wrap gap-1.5 items-center">
                        {lead.phoneNumbers.map((p, idx) => (
                          <div
                            key={p.id || idx}
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-white hover:bg-[#F2EFE8] border border-[#E4E0D6] text-[#1F2421] font-mono text-[10px]"
                          >
                            <DialLink
                              number={p.number}
                              leadId={lead.id}
                              onDial={() => onLaunchDialer(lead.id, p.number, true)}
                              className="flex items-center gap-1 text-[#1F2421] transition-colors cursor-pointer no-underline"
