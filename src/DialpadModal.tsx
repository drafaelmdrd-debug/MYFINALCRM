...
export function DialpadModal({
  ...
  // Active contact/owner being dialed
  const dialedPhoneRec = lead?.phoneNumbers.find(
    (p) =>
      p.number && phoneNumber && p.number.replace(/\D/g, '') === phoneNumber.replace(/\D/g, '')
  );
  const activeDialedName = dialedPhoneRec?.contactName || lead?.ownerName;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#A4AEA7]">
                {lead ? (
                  <>
                    <strong className="text-white font-bold">{activeDialedName}</strong>
                    {activeDialedName && activeDialedName.toLowerCase() !== lead.ownerName.toLowerCase() && (
                      <span className="text-[9px] text-[#E8A598] ml-1 font-semibold">
                        (Co-Owner • Primary: {lead.ownerName})
                      </span>
                    )}
